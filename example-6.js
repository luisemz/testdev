...
console.log('---- EXAMPLE 6 --- ', usersConcatObj(companies));

function usersConcatObj(companies) {
  const usersObjet = {};
  companies.forEach(function(company) {
    company.users.forEach(function(user) {
      user.company = company.name;
      usersObjet[`${user.firstName ? user.firstName : ''}${user.lastName ? user.lastName : ''}${user.age}`] = user.car;
    });
  });
  return usersObjet;
}
